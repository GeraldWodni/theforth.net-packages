...
\ from CORE :
\  ; execute 0= : bl word immediate drop environment? dup and BEGIN UNTIL 
\  2dup IF 2drop THEN EXIT rot postpone Literal recurse depth >r r> - 
\  Constant + = swap LOOP rshift over * j i 
\ from CORE-EXT :
\  :noname C" nip 0<> true false ?DO erase 
\ from CORE-EXT-2012 :
\  parse-name 
\ from BLOCK-EXT :
\  \ 
...
\  s" included ( 
\ from FLOAT :
\  d>f fdrop fdup f@ f* float+ f+ f! floats 
\ from LOCAL :
\  (local) 
\ from SEARCH :
\  find 
...
:noname c" [defined]" ; execute find 0= [if]
    s" extensions/defined.fs" included
[then]
drop
[undefined] {: [if]
...
: faxpy-nostride ( ra f_x f_y ucount -- )
	...
	fdup over f@ f* dup f@ f+ dup f! float+ swap float+ swap
    loop
    2drop fdrop ;
[then]
