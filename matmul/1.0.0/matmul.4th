...
:noname depth >r 0. d>f depth r> - >r fdrop r> ; execute
...
: matmul {: a b c ncols nrows -- :}
    \ C = A x B, where A has nrows rows and ncols cols (reversed for
    \ B), C has nrows rows and columns
    c nrows dup * floats erase
    nrows 0 ?do
	ncols 0 ?do \ c[j,.] += a[j,i]*b[i,.]
	    a j ncols * i + floats + f@
	    b i nrows * floats +
	    c j nrows * floats +
	    nrows faxpy-nostride
	loop
    loop ;
